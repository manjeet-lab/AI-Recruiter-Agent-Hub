...
class EducationItem(BaseModel):
    ...
    @classmethod
    def coerce_to_str(cls, values):
        if isinstance(values, dict):
            if 'graduation_year' in values and values['graduation_year'] is not None:
                values['graduation_year'] = str(values['graduation_year'])
        return values
...
class ResumeSchema(BaseModel):
    ...
    @model_validator(mode='before')
    @classmethod
    def coerce_fields_to_str(cls, values):
        if isinstance(values, dict):
            for key in ["Phone Number", "phone_number"]:
